...
requests.get is mocked throughout — no network calls. Added alongside a
fix for both functions: fetch_trades() called a nonexistent
...
def test_fetch_trades_uses_global_trades_path_with_ticker_filter():
    with patch("core.kalshi.requests.get", return_value=_resp({"trades": []})) as mock_get:
        kalshi.fetch_trades({"environment": "demo"}, "KXFOO-26JUN01")
    args, kwargs = mock_get.call_args
    assert args[0].endswith("/markets/trades")
    assert "/markets/KXFOO-26JUN01/trades" not in args[0]
...
def test_fetch_trades_returns_trade_list():
    ...
    with patch("core.kalshi.requests.get", return_value=_resp({"trades": trades})):
...
def test_fetch_orderbook_returns_response_unchanged():
    ...
    with patch("core.kalshi.requests.get", return_value=_resp(real_shape)):
...
def test_fetch_orderbook_uses_market_scoped_path():
    with patch("core.kalshi.requests.get", return_value=_resp({"orderbook_fp": {}})) as mock_get:
        kalshi.fetch_orderbook({"environment": "demo"}, "KXFOO-26JUN01")
    args, _ = mock_get.call_args
    assert args[0].endswith("/markets/KXFOO-26JUN01/orderbook")
